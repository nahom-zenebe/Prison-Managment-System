...
function AddCurrentInmate({ formData, setFormData, setImage }) {
  ...
  const getMinDate = () => {
    ...
    const minDate = new Date(
      ...
      currentDate.getMonth(),
      currentDate.getDate()
    ); // Subtract 30 years from current date
  ...
  const validateDateOfBirth = (_, value) => {
    ...
    if (
      ...
      selectedDate >= minDateForAdult ||
      selectedDate <= minDate
    ) {
      return Promise.reject(
        "Date of Birth must be a past date, and the person should be at least 18 years old, and not more than 100 years old."
      );
    }
    ...
    <div className="container">
      ...
      >
        <Form.Item
          ...
          rules={[{ required: true, message: "Please upload inmate photo" }]}
        >
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please enter Name with Initials (Ex:- John A B C)",
            },
        ...
        >
          <div className="InputName">
            <Input
              ...
              onChange={(e) => {
                setFormData({ ...formData, initialname: e.target.value });
              }}
            />
          </div>
        </Form.Item>
        ...
        >
          <div className="InputDropdown2">
            <Select
              className="InputFieldDropdown"
              placeholder="Select marital status"
              style={{ width: "100%" }}
              value={formData.marital}
              onChange={(value) => handleChange("marital", value)}
            >
